...
FFMPEG = "ffmpeg"
apiK = "0TzQIXtH26R9j4WSVWirvshBSphN0RqP"
searchArray = ['Dancing+Queen','Computer+Fun','So+Hot','So+Cute','So+Fun','The+Humberto','Film+Editors','Data+Fly','awesome','hooray','happy','your+the+best','explode','disney','socal','beach+days','so+cute','film','']
accepted_fileext = ['.jpg', '.dpx', '.png', '.tif', 'exr' ]
...
class offTransApp(QtGui.QMainWindow, design.Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.setupUi(self) 
        #### TAB 1.###################################################################
        self.v1_t1_btn.clicked.connect(lambda: self.browse_file(self.v1_t1,self.o1_t1))
        ...
        #### TAB 2.###################################################################
        self.v1_t2_btn.clicked.connect(lambda: self.browse_file(self.v1_t2,self.o1_t2))
        self.a1_t2_btn.clicked.connect(lambda: self.browse_file(self.a1_t2, None))
        self.o1_t2_btn.clicked.connect(lambda: self.browse_directory(self.o1_t2))
        self.transcode1_t2.clicked.connect(self.merge_av)
        #### TAB 2.###################################################################

        #### TAB 3.###################################################################
        self.v1_t3_btn.clicked.connect(lambda: self.browse_file(self.v1_t3,self.o1_t3))
        self.o1_t3_btn.clicked.connect(lambda: self.browse_directory(self.o1_t3))
        self.remove1_t3.clicked.connect(self.remove_audio)
        #### TAB 3.###################################################################

        #### TAB 4.###################################################################
        self.v1_t4_btn.clicked.connect(lambda: self.browse_file(self.v1_t4, self.o1_t4))
        self.o1_t4_btn.clicked.connect(lambda: self.save_file(self.o1_t4))
        self.extract_t4.clicked.connect(self.extract_audio)
        #### TAB 4.###################################################################

        #### TAB 5.###################################################################
        self.v1_t5_btn.clicked.connect(lambda: self.browse_file(self.v1_t5, self.o1_t5))
        self.a1_t5_btn.clicked.connect(lambda: self.browse_file(self.a1_t5, None))
    ...
    def make_gif(self):
        ...
        searchBuild = "http://api.giphy.com/v1/gifs/search?q={0}&api_key={1}&limit=20&rating=PG-13&lang=en".format(searchTerm,apiK)
        localGiph = "/giphs/thegipph.gif"
    ...
    def deduce_names(self,filePath):
        ...
        if fileExt in accepted_fileext:
            ...
            return {'wavName':wavName,'baseFileName' :baseFileName,'fullFolderPath' :fullFolderPath,'basicPath' :basicPath} 
    ...
    def movie_toframes(self):
        ...
        if self.cb1_t1.isChecked():
            ...
            ffmpegCmd = FFMPEG +" -i {0} -r {2} -q:v 2 {1}.%04d.jpg ".format(file,jpgName,fps)
    ...
    def browse_file(self,vpath,opath):
        ...
        if self.tabOptions.currentIndex() == 0:
                ...
                if self.mkdir_t1.isChecked():
                    ...
                    if not os.path.exists(namesDict['fullFolderPath']):
                        os.makedirs(unicode(namesDict['fullFolderPath']))
                        print("New directory made")  
                opath.setText(namesDict['fullFolderPath'])

        elif self.tabOptions.currentIndex() == 1:
            opath.setText(namesDict['basicPath'] + namesDict['baseFileName'] + "_WITH_AUDIO" + file[-4:] ) 
            ...
            opath.setText(namesDict['basicPath'] + namesDict['baseFileName'] + "_NO_AUDIO" + file[-4:] )
